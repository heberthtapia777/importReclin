      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
          <section class="wrapper">
            <h1 class="avisos" align="center"><strong>LISTA DE VENTAS</strong></h1>
            <?php
              if($_GET['ope']='ope'){
                $ope="OPERADOR";
              }
            ...
            ?>
          	<h3><i class="fa fa-angle-right"></i> <?=$ope.': '.$_GET['nombre']; ?></h3>
        <div class="row mt">
          <div class="col-lg-12">
            <div class="content-panel">
              ...
              <section id="unseen">
                <table id="tablaList" class="table table-bordered table-striped table-condensed">
                  ...
                  $swEmpleado=0;
                  $sql  = "SELECT *, c.dateReg AS fecha FROM compra AS c, empleado AS e,comprarepuesto AS cr, repuesto AS r
                  WHERE c.id_empleado = e.id_empleado AND c.id_compra = cr.id_compra AND cr.id_repuesto = r.id_repuesto ";
                 
                  if($cargo != 'adm'){
                    $sql .= "AND e.id_empleado = '".$_SESSION['idEmp']."' AND c.dateReg between CONCAT ( '".$FECHAINICIO."' , ' 00:00:00' ) and CONCAT( '".$FECHAFIN."' , ' 23:59:59')";
                    $swEmpleado=1;
                  }
                  if($todos==''){
                    $sql.="AND r.name like '%".$nombreRepuestos."%'";
                  }
                  $sql  .= " AND e.id_empleado = '".$_GET['id']."' AND c.dateReg between CONCAT ( '".$FECHAINICIO."' , ' 00:00:00' ) and CONCAT( '".$FECHAFIN."' , ' 23:59:59') ORDER BY (c.id_compra) DESC";
                 //echo $sql; die();
                  ?>
